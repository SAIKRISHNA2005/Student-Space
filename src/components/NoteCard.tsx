...
const NoteCard = ({ title, subtitle, date, teacher, important }: NoteProps) => {
  ...
  return (
    <div className={`p-4 rounded-lg border ${isBookmarked ? "bg-blue-600 text-white" : "bg-white"} shadow-md flex flex-col h-full`}>
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-2">
          <FontAwesomeIcon icon={faBookOpen} className={`text-lg ${isBookmarked ? "text-white" : "text-blue-600"}`} />
          <h3 className={`text-lg font-semibold ${isBookmarked ? "text-white" : "text-blue-900"}`}>{title}</h3>
        </div>
        <FontAwesomeIcon
          icon={faBookmark}
          className={`text-lg cursor-pointer ${isBookmarked ? "text-white" : "text-blue-600"}`}
