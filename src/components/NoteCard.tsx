...
const NoteCard = ({ title, subtitle, date, teacher, important }: NoteProps) => {
  ...
  return (
    <div className={`p-4 rounded-lg border ${isBookmarked ? "bg-[#1E3A8A] text-white" : "bg-white"} shadow-md flex flex-col h-full`}>
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-2">
          <FontAwesomeIcon icon={faBookOpen} className={`text-lg ${isBookmarked ? "text-white" : "text-[#1E3A8A]"}`} />
          <h3 className={`text-lg font-semibold ${isBookmarked ? "text-white" : "text-[#1E3A8A]"}`}>{title}</h3>
        </div>
        <FontAwesomeIcon
          icon={faBookmark}
          className={`text-lg cursor-pointer ${isBookmarked ? "text-white" : "text-[#1E3A8A]"}`}
